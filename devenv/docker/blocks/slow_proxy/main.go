...
func main() {
	...
	if origin == "" {
		...
		slog.Error("Missing env-variable", "variable", "ORIGIN_SERVER")
		os.Exit(1)
	}
	...
	if sleepDurationStr == "" {
		// it is never not-set, the default is in the `.env` file
		slog.Error("Missing env-variable", "variable", "SLEEP_DURATION")
		os.Exit(1)
	}

	sleep, err := time.ParseDuration(sleepDurationStr)
	if err != nil {
		slog.Error("Failed to parse SLEEP_DURATION", "error", err)
		os.Exit(1)
	}
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		slog.Info("Proxying request after sleep", "sleepDuration", safeSleep, "url", safeRequestUri)

		// This is commented out as CodeQL flags this as vulnerability CWE-117 (https://cwe.mitre.org/data/definitions/117.html)
		// If you need to debug and log the headers then use the line below instead of the slog.Info statement above
		...
		//
		// slog.Info("Proxying request after sleep", "sleepDuration", safeSleep, "url", safeRequestUri, "headers", r.Header)
		<-time.After(sleep)
		proxy.ServeHTTP(w, r)
	})
