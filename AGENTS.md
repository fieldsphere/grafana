...
- **Node.js** — version pinned in `.nvmrc` (currently `v24.11.0`), installed via nvm and set as the nvm default. **Run the frontend via a login shell** (tmux session or `bash -lc '...'`): `yarn start` / webpack **require the pinned Node 24** because the plugin `webpack.config.ts` files import sibling `.ts` modules using native TypeScript support; the infra-injected `/exec-daemon/node` (v22) is missing this and fails with `ERR_UNKNOWN_FILE_EXTENSION`. **PATH gotcha:** `/exec-daemon/node` is prepended ahead of nvm, so non-login shells resolve `node` to v22. A line appended to `~/.bashrc` re-prepends the nvm default bin so login shells resolve `node` to the pinned v24 (and to the corepack `yarn` shim). If `node -v` in a login shell is not v24, run `nvm install` (reads `.nvmrc`), `nvm alias default <version>`, and `corepack enable`.
