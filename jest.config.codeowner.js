...
if (!codeownerName) {
  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'jest.config.codeowner.js', args: ['ERROR: CODEOWNER_NAME environment variable is required'] }]);
  process.exit(1);
}
...
if (!fs.existsSync(codeownersFilePath)) {
  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'jest.config.codeowner.js', args: [`Codeowners file not found at ${codeownersFilePath} ...`] }]);
  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'jest.config.codeowner.js', args: ['Please run: yarn codeowners-manifest first to generate the mapping file'] }]);
  process.exit(1);
}
...
if (teamFiles.length === 0) {
  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'jest.config.codeowner.js', args: [`ERROR: No files found for team "${codeownerName}"`] }]);
  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'jest.config.codeowner.js', args: ['Available teams:', Object.keys(codeownersData).join(', ')] }]);
  process.exit(1);
}
...
if (testFiles.length === 0) {
  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'jest.config.codeowner.js', args: [`No test files found for team ${codeownerName}`] }]);
  process.exit(0);
}

Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'jest.config.codeowner.js', args: [
    ...
    [
      'jest-monocart-coverage',
      {
        ...
        onEnd: (coverageResults) => {
          ...
          Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'jest.config.codeowner.js', args: [`📄 Coverage report saved to ${reportURL}`] }]);
...
function writeCoverageSummaryArtifact(coverageResults) {
  ...
  try {
    ...
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'jest.config.codeowner.js', args: [`📊 Coverage summary written to ${COVERAGE_SUMMARY_OUTPUT_PATH}`] }]);
  } catch (err) {
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'jest.config.codeowner.js', args: [`Failed to write coverage summary: ${err}`] }]);
  }
}
...
async function openCoverageReport(reportURL) {
  try {
    ...
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'jest.config.codeowner.js', args: [`Failed to open coverage report: ${err}`] }]);
  }
}
