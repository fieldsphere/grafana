...
class LogReporter extends Mocha.reporters.Spec {
  ...
  reportStats() {
    ...
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'e2e/log-reporter.js', args: [`CypressStats ${objToLogAttributes(stats)}`] }]);
  }
  ...
  reportResults() {
    this._testsResults.map(cleanTest).map((test) => {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'e2e/log-reporter.js', args: [`CypressTestResult ${objToLogAttributes(test)}`] }]);
    });
  }
  ...
  reportErrors() {
    this._testFailures.map(cleanTest).forEach((failure) => {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'e2e/log-reporter.js', args: [`CypressError ${objToLogAttributes({ suite, test, error })}`] }]);
    });
  }
}
