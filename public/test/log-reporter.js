class LogReporter {
  ...
  logStats(results) {
    ...
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/test/log-reporter.js', args: [`JestStats ${objToLogAttributes(stats)}`] }]);
  }
}
...
function printTestFailures(result) {
  ...
  if (result.numFailingTests > 0) {
    ...
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/test/log-reporter.js', args: [`JestFailure ${objToLogAttributes(testInfo)}`] }]);
  }
}
