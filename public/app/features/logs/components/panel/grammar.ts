...
export const generateTextMatchGrammar = (highlightWords: string[] | undefined = [], search?: string): Grammar => {
  ...
  const expressions = highlightWords
    .map((word) => {
      ...
      try {
        ...
        structuredLogFromConsole('error', `generateTextMatchGrammar: cannot generate regular expression from /${cleaned}/${flags}`, e);
      }
      return undefined;
    })
  ...
  if (search) {
    try {
      ...
      structuredLogFromConsole('error', `generateTextMatchGrammar: cannot generate regular expression from /${search}/gi`, e);
    }
  }
