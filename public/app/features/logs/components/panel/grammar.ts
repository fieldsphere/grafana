...
export const generateTextMatchGrammar = (highlightWords: string[] | undefined = [], search?: string): Grammar => {
  ...
  const expressions = highlightWords
    .map((word) => {
      ...
      try {
        ...
        (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error(`generateTextMatchGrammar: cannot generate regular expression from /${cleaned}/${flags}`, e);
      }
      return undefined;
    })
  ...
  if (search) {
    try {
      ...
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error(`generateTextMatchGrammar: cannot generate regular expression from /${search}/gi`, e);
    }
  }
