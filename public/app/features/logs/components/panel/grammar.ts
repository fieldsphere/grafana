import {
  type Grammar } from 'prismjs';

import { escapeRegex, parseFlags,
  structuredLog,
  toLogContextPart
} from '@grafana/data';
...
export const generateTextMatchGrammar = (highlightWords: string[] | undefined = [], search?: string): Grammar => {
  ...
  const expressions = highlightWords
    .map((word) => {
      ...
      try {
        ...
        structuredLog('error', `generateTextMatchGrammar: cannot generate regular expression from /${cleaned}/${flags}`, { error: toLogContextPart(e) });
      }
      return undefined;
    })
  ...
  if (search) {
    try {
      ...
      structuredLog('error', `generateTextMatchGrammar: cannot generate regular expression from /${search}/gi`, { error: toLogContextPart(e) });
    }
  }
