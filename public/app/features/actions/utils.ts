...
const logger = createMonitoringLogger('features.actions.utils');
      ...
      const actionModel: ActionModel<Field> = {
        ...
        onClick: (evt: MouseEvent, origin: Field, actionVars?: ActionVariableInput) => {
          ...
          try {
              ...
              .subscribe({
                error: (error) => {
                  ...
                  if (error instanceof Error) {
                    logger.logError(error, { operation: 'action.onClick.fetch.subscribe' });
                  } else {
                    logger.logWarning('Action fetch request failed', {
                      operation: 'action.onClick.fetch.subscribe',
                      error: String(error),
                    });
                  }
                },
            ...
            if (error instanceof Error) {
              logger.logError(error, { operation: 'action.onClick.fetch' });
            } else {
              logger.logWarning('Action request failed', { operation: 'action.onClick.fetch', error: String(error) });
            }
            return;
          }
        },
