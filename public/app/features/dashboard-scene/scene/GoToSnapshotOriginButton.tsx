...
const logger = createMonitoringLogger('features.dashboard-scene.snapshot-origin-button');
...
export const onOpenSnapshotOriginalDashboard = (originalUrl: string) => {
  ...
  try {
    ...
    if (err instanceof Error) {
      logger.logError(err, { operation: 'onOpenSnapshotOriginalDashboard', originalUrl });
    } else {
      logger.logWarning('Failed to open original dashboard', {
        operation: 'onOpenSnapshotOriginalDashboard',
        originalUrl,
        error: String(err),
      });
    }
