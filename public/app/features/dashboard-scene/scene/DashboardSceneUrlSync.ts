...
const logger = createMonitoringLogger('features.dashboard-scene.url-sync');
...
export class DashboardSceneUrlSync implements SceneObjectUrlSyncHandler {
  ...
  updateFromUrl(values: SceneObjectUrlValues): void {
    ...
    if (typeof values.editPanel === 'string') {
      ...
      if (!panel) {
        logger.logWarning('Panel not found when handling editPanel URL state', { panelKey: values.editPanel });
        return;
      }
