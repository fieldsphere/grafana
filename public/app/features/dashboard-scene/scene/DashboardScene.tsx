...
export class DashboardScene extends SceneObjectBase<DashboardSceneState> implements LayoutParent {
  ...
  public exitEditMode({ skipConfirm, restoreInitialState }: { skipConfirm: boolean; restoreInitialState?: boolean }) {
    if (!this.canDiscard()) {
      structuredLog('error', 'Trying to discard back to a state that does not exist, initialState undefined');
      return;
    }
  ...
  public copyPanel(vizPanel: VizPanel) {
    if (config.featureToggles.dashboardNewLayouts) {
      ...
      if (gridItem instanceof AutoGridItem) {
        ...
        structuredLog('error', 'Trying to copy a panel that is not DashboardGridItem child');
  ...
  public pastePanelStyles(vizPanel: VizPanel) {
    ...
    try {
      ...
      structuredLog('error', 'Error pasting panel styles:', { error: toLogContextPart(e) });
  ...
  public unlinkLibraryPanel(panel: VizPanel) {
    ...
    structuredLog('error', 'Trying to unlink a lib panel in a layout that is not DashboardGridItem or AutoGridItem');
  }
