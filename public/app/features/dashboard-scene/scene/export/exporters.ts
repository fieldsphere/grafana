...
const logger = createMonitoringLogger('grafana.features.dashboard-scene.scene.export.exporters');
import {
...
export async function makeExportableV1(dashboard: DashboardModel) {
  ...
  try {
    ...
    logger.logError('Export failed:', err);
    return {
      error: err,
    };
  }
...
async function convertLibraryPanelToInlinePanel(libraryPanelElement: LibraryPanelKind): Promise<PanelKind> {
  ...
  try {
    ...
    logger.logError(`Failed to load library panel ${libraryPanel.uid}:`, error);
