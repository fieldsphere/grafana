...
const logger = createMonitoringLogger('grafana.features.dashboard-scene.scene.layout-default.RowRepeaterBehavior');
...
export class RowRepeaterBehavior extends SceneObjectBase<RowRepeaterBehaviorState> {
  ...
  public performRepeat(force = false) {
    ...
    if (!variable) {
      logger.logError(new Error('RepeatedRowBehavior: Variable not found'));
      return;
    }

    if (!(variable instanceof MultiValueVariable)) {
      logger.logError(new Error('RepeatedRowBehavior: Variable is not a MultiValueVariable'));
      return;
    }
