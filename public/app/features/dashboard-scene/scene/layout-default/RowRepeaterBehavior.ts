...
export class RowRepeaterBehavior extends SceneObjectBase<RowRepeaterBehaviorState> {
  ...
  public performRepeat(force = false) {
    ...
    if (!variable) {
      structuredLogFromConsole('error', 'RepeatedRowBehavior: Variable not found');
      return;
    }

    if (!(variable instanceof MultiValueVariable)) {
      structuredLogFromConsole('error', 'RepeatedRowBehavior: Variable is not a MultiValueVariable');
      return;
    }
