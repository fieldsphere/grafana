...
import { createStructuredLogger, TimeRangeUpdatedEvent } from '@grafana/runtime';

const logger = createStructuredLogger('DashboardModelCompatibilityWrapper');
...
export class DashboardModelCompatibilityWrapper {
  ...
  public removePanel(panel: PanelModelCompatibilityWrapper) {
    ...
    if (!vizPanel) {
      logger.error('Trying to remove a panel that was not found in scene', { panelId: panel.id });
      return;
    }
