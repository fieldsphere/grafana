...
const logger = createMonitoringLogger('grafana.features.dashboard-scene.utils.PanelModelCompatibilityWrapper');
...
export class PanelModelCompatibilityWrapper implements PanelModel {
  ...
  public get id() {
    ...
    if (isNaN(id)) {
      logger.logError('VizPanel key could not be translated to a legacy numeric panel id', this._vizPanel);
      return 0;
    }
