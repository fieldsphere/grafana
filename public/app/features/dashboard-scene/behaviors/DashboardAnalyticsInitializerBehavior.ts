...
const logger = createMonitoringLogger('grafana.features.dashboard-scene.behaviors.DashboardAnalyticsInitializerBehavior');
...
export function dashboardAnalyticsInitializer(dashboard: DashboardScene) {
  ...
  if (!uid) {
    logger.logWarning('dashboardAnalyticsInitializer: Dashboard UID is missing');
    return;
  }
