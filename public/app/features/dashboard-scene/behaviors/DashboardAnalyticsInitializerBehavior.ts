...
export function dashboardAnalyticsInitializer(dashboard: DashboardScene) {
  ...
  if (!uid) {
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'public/app/features/dashboard-scene/behaviors/DashboardAnalyticsInitializerBehavior.ts', args: ['dashboardAnalyticsInitializer: Dashboard UID is missing'] }]);
    return;
  }
