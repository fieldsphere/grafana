...
const logger = createMonitoringLogger('features.dashboard-scene.pages-utils');
...
export async function updateNavModel(folderUid: string) {
  try {
    ...
    if (err instanceof Error) {
      logger.logError(err, { operation: 'updateNavModel', folderUid });
      return;
    }
    logger.logWarning('Error fetching parent folder for dashboard', {
      operation: 'updateNavModel',
      folderUid,
      error: String(err),
    });
  }
