...
const logger = createMonitoringLogger('grafana.features.alerting.unified.settings.extensions');
...
export function addSettingsSection(pageNav: SettingsSectionNav) {
  if (settingsExtensions.has(pageNav.url)) {
    logger.logWarning('Unable to add settings page, PageNav must have an unique url');
    return;
  }
