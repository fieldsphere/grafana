...
const logger = createMonitoringLogger('grafana.features.alerting.unified.state.AlertingQueryRunner');
...
const getTimeRange = (query: AlertQuery, queries: AlertQuery[]): TimeRange => {
  ...
  if (!query.relativeTimeRange) {
    logger.logWarning(`Query with refId: ${query.refId} did not have any relative time range, using default.`);
