...
const logger = createMonitoringLogger('features.scopes.api-client');
...
export class ScopesApiClient {
  ...
  private extractDataOrHandleError<T>(result: { data?: T; error?: unknown }, context: string): T | undefined {
    if ('data' in result && result.data) {
      ...
      if (this.isStatusError(result.data)) {
        ...
        logger.logWarning('Failed to fetch resource', {
          operation: 'extractDataOrHandleError',
          context,
          error: errorMessage,
        });
        return undefined;
  ...
  async fetchScope(name: string): Promise<Scope | undefined> {
    ...
    try {
      ...
      logger.logWarning('Failed to fetch scope', { operation: 'fetchScope', scopeName: name, error: errorMessage });
      return undefined;
    } finally {
  ...
  async fetchMultipleScopes(scopesIds: string[]): Promise<Scope[]> {
    ...
    try {
      ...
      if (successfulScopes.length < scopesIds.length) {
        ...
        logger.logWarning('Failed to fetch all requested scopes', {
          operation: 'fetchMultipleScopes',
          failedCount,
      ...
      const errorMessage = getMessageFromError(err);
      logger.logWarning('Failed to fetch multiple scopes', {
        operation: 'fetchMultipleScopes',
        scopeIds: scopesIds.join(','),
        error: errorMessage,
      });
  ...
  async fetchMultipleScopeNodes(names: string[]): Promise<ScopeNode[]> {
    ...
    try {
      ...
      if ('error' in result) {
        const errorMessage = getMessageFromError(result.error);
        logger.logWarning('Failed to fetch multiple scope nodes', {
          operation: 'fetchMultipleScopeNodes',
          names: names.join(','),
          error: errorMessage,
        });
      }
  ...
  async fetchNodes(options: { parent?: string; query?: string; limit?: number }): Promise<ScopeNode[]> {
    ...
    try {
      ...
      if ('error' in result) {
        ...
        logger.logWarning('Failed to fetch scope nodes', { operation: 'fetchNodes', context, error: errorMessage });
      }
  ...
  public fetchDashboards = async (scopeNames: string[]): Promise<ScopeDashboardBinding[]> => {
    ...
    try {
      ...
      if ('error' in result) {
        const errorMessage = getMessageFromError(result.error);
        logger.logWarning('Failed to fetch dashboards for scopes', {
          operation: 'fetchDashboards',
  ...
  public fetchScopeNavigations = async (scopeNames: string[]): Promise<ScopeNavigation[]> => {
    ...
    try {
      ...
      if ('error' in result) {
        const errorMessage = getMessageFromError(result.error);
        logger.logWarning('Failed to fetch scope navigations for scopes', {
          operation: 'fetchScopeNavigations',
          scopeNames: scopeNames.join(','),
          error: errorMessage,
        });
      }
  ...
  public fetchScopeNode = async (scopeNodeId: string): Promise<ScopeNode | undefined> => {
    ...
    try {
      ...
      const errorMessage = getMessageFromError(err);
      logger.logWarning('Failed to fetch scope node', {
        operation: 'fetchScopeNode',
        scopeNodeId,
        error: errorMessage,
      });
      return undefined;
