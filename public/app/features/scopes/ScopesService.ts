...
import { createStructuredLogger, LocationService, ScopesContextValue, ScopesContextValueState } from '@grafana/runtime';

const logger = createStructuredLogger('ScopesService');
  ...
  constructor(
    ...
    if (nodeToPreload) {
      this.selectorService.resolvePathToRoot(nodeToPreload, this.selectorService.state.tree!).catch((error) => {
        logger.error('Failed to pre-load node path', error instanceof Error ? error : undefined, { nodeToPreload });
      });
    }
