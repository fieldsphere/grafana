...
const logger = createMonitoringLogger('grafana.features.scopes.selector.scopesTreeUtils');
...
export const insertPathNodesIntoTree = (tree: TreeNode, path: ScopeNode[]) => {
  ...
  for (let index = 0; index < stringPath.length - 1; index++) {
    ...
    newTree = modifyTreeNodeAtPath(newTree, pathSlice, (treeNode) => {
      ...
      if (!childNodeName) {
        logger.logWarning('Failed to insert full path into tree. Did not find child to' + stringPath[index]);
