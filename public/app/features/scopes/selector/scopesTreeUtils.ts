...
const logger = createMonitoringLogger('features.scopes.selector.tree-utils');
...
export const insertPathNodesIntoTree = (tree: TreeNode, path: ScopeNode[]) => {
  ...
  for (let index = 0; index < stringPath.length - 1; index++) {
    ...
    newTree = modifyTreeNodeAtPath(newTree, pathSlice, (treeNode) => {
      ...
      if (!childNodeName) {
        logger.logWarning('Failed to insert full path into tree. Missing child node.', {
          operation: 'insertPathNodesIntoTree',
          parentSegment: stringPath[index],
        });
