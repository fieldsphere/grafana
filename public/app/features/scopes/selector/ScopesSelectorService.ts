...
export class ScopesSelectorService extends ScopesServiceBase<ScopesSelectorServiceState> {
  ...
  public getScopeNode = async (scopeNodeId: string) => {
    ...
    try {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'public/app/features/scopes/selector/ScopesSelectorService.ts', args: ['Failed to load node', error] }]);
      return undefined;
    }
  };
  ...
  private getNodePath = async (scopeNodeId: string, visited: Set<string> = new Set()): Promise<ScopeNode[]> => {
    ...
    if (visited.has(scopeNodeId)) {
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'public/app/features/scopes/selector/ScopesSelectorService.ts', args: ['Circular reference detected in node path', scopeNodeId] }]);
      return [];
    }
  ...
  private applyScopes = async (scopes: SelectedScope[], redirectOnApply = true) => {
    ...
    if (scopes.length > 0) {
      ...
      if (!Array.isArray(fetchedScopes)) {
        Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'public/app/features/scopes/selector/ScopesSelectorService.ts', args: ['Expected fetchedScopes to be an array, got:', typeof fetchedScopes] }]);
  ...
  public open = async () => {
    ...
    if (this.state.selectedScopes.length && this.state.selectedScopes[0].scopeNodeId) {
      try {
        ...
        Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'public/app/features/scopes/selector/ScopesSelectorService.ts', args: ['Failed to expand to selected scope', error] }]);
      }
    }
...
function parseScopesFromLocalStorage(content: string | undefined): RecentScope[][] {
  ...
  try {
    ...
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'public/app/features/scopes/selector/ScopesSelectorService.ts', args: ['Failed to parse recent scopes', e, content] }]);
    return [];
  }
