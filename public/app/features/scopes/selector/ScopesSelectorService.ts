...
import { createMonitoringLogger, config, locationService } from '@grafana/runtime';

const logger = createMonitoringLogger('grafana.features.scopes.selector.ScopesSelectorService');
...
export class ScopesSelectorService extends ScopesServiceBase<ScopesSelectorServiceState> {
  ...
  private getNodePath = async (scopeNodeId: string, visited: Set<string> = new Set()): Promise<ScopeNode[]> => {
    ...
    if (visited.has(scopeNodeId)) {
      logger.logError('Circular reference detected in node path', { scopeNodeId });
      return [];
    }
  ...
  private applyScopes = async (scopes: SelectedScope[], redirectOnApply = true) => {
    ...
    if (scopes.length > 0) {
      ...
      if (!Array.isArray(fetchedScopes)) {
        logger.logError('Expected fetchedScopes to be an array, got:', { type: typeof fetchedScopes });
  ...
  public open = async () => {
    ...
    if (this.state.selectedScopes.length && this.state.selectedScopes[0].scopeNodeId) {
      try {
        ...
        logger.logError('Failed to expand to selected scope', { error });
      }
    }
...
function parseScopesFromLocalStorage(content: string | undefined): RecentScope[][] {
  ...
  try {
    ...
    logger.logError('Failed to parse recent scopes', { error: e, content });
    return [];
  }
