...
import { getGrafanaLiveSrv, locationService, logStructuredDebug } from '@grafana/runtime';
        ...
        switch (action) {
          ...
          case DashboardEventAction.Saved: {
            ...
            if (dash?.uid !== event.message.uid) {
              logStructuredDebug('features.live', 'dashboard event for different dashboard?', event, dash);
              return;
            }
