...
const logger = createMonitoringLogger('grafana.features.live.centrifuge.LiveDataStream');
...
export class LiveDataStream<T = unknown> {
  ...
  private onError = (err: unknown) => {
    logger.logDebug('LiveQuery [error]', { err, channelId: this.deps.channelId });
    this.stream.next({
  ...
  private onComplete = () => {
    logger.logDebug('LiveQuery [complete]', { channelId: this.deps.channelId });
    this.shutdown();
  };
  ...
  get = (options: LiveDataStreamOptions, subKey: DataStreamSubscriptionKey): Observable<StreamingDataQueryResponse> => {
    ...
    const getFullFrameResponseData = <T>(
      ...
      if (!messages.length) {
        logger.logWarning(`expected to find at least one non error message ${messages.map(({ type }) => type)}`);
        // send empty frame
        return {
          key: subKey,
    ...
    const transformedInternalStream = this.stream.pipe(
      ...
      map((messages, i) => {
        ...
        if (newValueSameSchemaMessages.length !== messages.length) {
          logger.logWarning(`unsupported message type ${messages.map(({ type }) => type)}`);
        }
