...
export class ExtensionsLog {
  ...
  warning(message: string, labels?: Labels): void {
    ...
    config.buildInfo.env === 'development' && pluginsLogger.warn(message, { ...this.baseLabels, ...labels });
  ...
  error(message: string, labels?: Labels): void {
    ...
    // (first: `monitoringLogger.logError()`, second: `pluginsLogger.error()` which gets picked up by Faro)
    monitoringLogger.logError(new Error(message), { ...this.baseLabels, ...labels });
    pluginsLogger.error(new Error(message), { ...this.baseLabels, ...labels });
