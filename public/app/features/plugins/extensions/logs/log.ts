...
import { config, createMonitoringLogger, logWarning as logWarningRuntime, logError as logErrorRuntime } from '@grafana/runtime';
...
export class ExtensionsLog {
  ...
  warning(message: string, labels?: Labels): void {
    ...
    config.buildInfo.env === 'development' && logWarningRuntime(message, { ...this.baseLabels, ...labels });
  ...
  error(message: string, labels?: Labels): void {
    ...
    // (first: `monitoringLogger.logError()`, second: `logErrorRuntime()` which gets picked up by Faro)
    monitoringLogger.logError(new Error(message), { ...this.baseLabels, ...labels });
    logErrorRuntime(new Error(message), { ...this.baseLabels, ...labels });
