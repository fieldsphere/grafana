...
const logger = createMonitoringLogger('grafana.features.plugins.extensions.logs.LogViewFilters');
...
function useLogFilters(
  ...
  return useMemo(() => {
    if (data && data?.series.length > 1) {
      logger.logWarning('LogViewFilter does not support multiple series in query result.', {
