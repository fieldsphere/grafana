...
const logger = createMonitoringLogger('grafana.features.plugins.importer.addTranslationsToI18n');
...
export async function addTranslationsToI18n({
  ...
  if (!path) {
    logger.logWarning(`Could not find any translation for plugin ${pluginId}`, {
      pluginId,
      resolvedLanguage,
      fallbackLanguage,
    });
  ...
  try {
    ...
    if (!module.default) {
      logger.logWarning(`Could not find default export for plugin ${pluginId}`, {
        pluginId,
        resolvedLanguage,
        fallbackLanguage,
        path,
    ...
    logger.logWarning(`Could not load translation for plugin ${pluginId}`, {
      pluginId,
      resolvedLanguage,
      fallbackLanguage,
