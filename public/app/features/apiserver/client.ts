    ...
    if (this.gvr.group === 'provisioning.grafana.app') {
      ...
      return getGrafanaLiveSrv()
        ...
        .pipe(
          ...
          catchError((error) => {
            (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error('Live channel watch stream error:', error);
            throw error;
          })
        );
    }
      ...
      .pipe(
        ...
        map((line) => {
          try {
            ...
            (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).warn('Invalid JSON in watch stream:', e, line);
            return null;
          }
        }),
        ...
        catchError((error) => {
          (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error('Watch stream error:', error);
          throw error;
        })
      );
  }
