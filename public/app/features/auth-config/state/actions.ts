...
const logger = createMonitoringLogger('features.auth-config.state-actions');
    ...
    if (contextSrv.hasPermission(AccessControlAction.SettingsWrite)) {
      try {
        ...
        if (error instanceof Error) {
          ...
          logger.logWarning('Failed to save settings', { operation: 'saveSettings', error: String(error) });
        }
