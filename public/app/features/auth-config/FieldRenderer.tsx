...
export const FieldRenderer = ({
  ...
  if (!field) {
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/app/features/auth-config/FieldRenderer.tsx', args: ['missing field:', name] }]);
    return null;
  }
  ...
  switch (fieldData.type) {
    ...
    default:
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'public/app/features/auth-config/FieldRenderer.tsx', args: [`Unknown field type: ${fieldData.type}`] }]);
      return null;
  }
};
