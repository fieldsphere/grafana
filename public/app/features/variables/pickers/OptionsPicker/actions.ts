...
const logger = createMonitoringLogger('features.variables.options-picker-actions');
...
const searchForOptions = async (
  ...
  try {
    ...
    if (error instanceof Error) {
      logger.logError(error, { operation: 'searchForOptions', key, searchQuery });
    } else {
      logger.logWarning('Failed to search variable options', {
        operation: 'searchForOptions',
        key,
        searchQuery,
        error: String(error),
      });
