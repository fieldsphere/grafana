...
export const onTimeRangeUpdated =
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'public/app/features/variables/state/actions.ts', args: [error] }]);
...
export const initVariablesTransaction =
  ...
  async (dispatch, getState) => {
    try {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'public/app/features/variables/state/actions.ts', args: [err] }]);
    }
  };
