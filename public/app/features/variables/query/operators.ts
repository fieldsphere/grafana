...
const logger = createMonitoringLogger('features.variables.query-operators');
    ...
    source.pipe(
      map((results) => {
        ...
        if (!variable.rootStateKey) {
          logger.logWarning('updateOptionsState: variable.rootStateKey is not defined', { variableName: variable.name });
          return;
        }
