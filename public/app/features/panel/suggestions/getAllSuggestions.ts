...
const logger = createMonitoringLogger('features.panel.suggestions');
...
export async function loadPlugins(pluginIds: string[]): Promise<PluginLoadResult> {
  ...
  for (let i = 0; i < settledPromises.length; i++) {
    ...
    if (settled.status === 'fulfilled') {
      ...
      logger.logWarning('Failed to load panel plugin for visualization suggestions', {
...
export async function getAllSuggestions(series?: DataFrame[]): Promise<SuggestionsResult> {
  ...
  for (const plugin of plugins) {
    try {
      ...
      logger.logWarning('Error when loading suggestions from plugin', {
