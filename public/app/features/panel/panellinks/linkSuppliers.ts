...
const logger = createMonitoringLogger('features.panel.link-suppliers');
  ...
  return {
    getLinks: (replaceVariables: InterpolateFunction) => {
      ...
      if (value.view) {
        ...
        logger.logDebug('Unexpected value without field context in data link', { valueType: typeof value });
      }
