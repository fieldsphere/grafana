...
const logger = createMonitoringLogger('features.explore.logs-table-wrap');
...
export function LogsTableWrap(props: Props) {
  ...
  const toggleColumn = (columnName: FieldName) => {
    if (!columnsWithMeta || !(columnName in columnsWithMeta)) {
      logger.logWarning('Failed to get column', {
        columnName,
        availableColumns: columnsWithMeta ? Object.keys(columnsWithMeta) : [],
      });
      return;
    }
