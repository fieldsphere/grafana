...
const logger = createMonitoringLogger('features.explore.state.correlations');
      ...
      await createCorrelation(sourceDatasource.uid, correlation)
        ...
        .catch((err) => {
          ...
          if (err instanceof Error) {
            logger.logError(err, { operation: 'addCorrelation' });
            return;
          }
          logger.logWarning('Error creating correlation', {
            operation: 'addCorrelation',
            error: String(err),
          });
        });
    }
