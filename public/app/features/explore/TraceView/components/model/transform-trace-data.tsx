...
const logger = createMonitoringLogger('features.explore.trace-transform-data');
...
export default function transformTraceData(data: TraceResponse | undefined): Trace | null {
  ...
  for (let i = 0; i < max; i++) {
    ...
    if (idCount != null) {
      logger.logWarning('Duplicate spanID encountered', {
        spanID,
        duplicateCount: idCount + 1,
      });
      if (_isEqual(span, spanMap.get(spanID))) {
        logger.logWarning('Two spans with same ID have isEqual(...) === true', { spanID });
      }
