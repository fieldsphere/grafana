...
import { createStructuredLogger, toDataQueryError } from '@grafana/runtime';
import { ExpressionDatasourceRef } from '@grafana/runtime/internal';

const logger = createStructuredLogger('PanelQueryRunner');
...
export class PanelQueryRunner {
  ...
  private applyTransformations(data: PanelData): Observable<PanelData> {
    ...
    return forkJoin([seriesStream, annotationsStream]).pipe(
      ...
      catchError((err) => {
        logger.warn('Error running transformation', { error: err instanceof Error ? err.message : String(err) });
        return of({
          ...data,
