...
import { config, createMonitoringLogger, toDataQueryError } from '@grafana/runtime';
...
const logger = createMonitoringLogger('features.query.dashboard-query-runner-utils');
...
function notifyWithError(title: string, err: any) {
  ...
  logger.logError(error, { operation: 'notifyWithError', title });
