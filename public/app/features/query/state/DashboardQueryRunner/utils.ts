...
import { config, createStructuredLogger, toDataQueryError } from '@grafana/runtime';

const logger = createStructuredLogger('DashboardQueryRunnerUtils');
...
function notifyWithError(title: string, err: any) {
  ...
  logger.error('handleAnnotationQueryRunnerError', err instanceof Error ? err : undefined, { title, message: error.message });
