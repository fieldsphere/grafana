...
import { createStructuredLogger, useChromeHeaderHeight } from '@grafana/runtime';
import { CodeEditor, Combobox, Field, Stack, useStyles2 } from '@grafana/ui';

const logger = createStructuredLogger('ThemePlayground');
...
for (const [name, json] of Object.entries(experimentalDefinitions)) {
  ...
  if (!result.success) {
    logger.error('Invalid theme definition', undefined, { themeName: name, error: result.error.message });
  } else {
