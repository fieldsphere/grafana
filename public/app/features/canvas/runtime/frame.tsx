...
const logger = createStructuredLogger('CanvasFrame');
    ...
    switch (action) {
      ...
      case LayerActionID.Duplicate:
        if (element.item.id === 'frame') {
          logger.info('Can not duplicate frames (yet)', { action, elementName: element.options.name });
          return;
        }
      ...
      default:
        logger.info('DO action', { action, elementName: element.options.name });
        return;
    }
  };
