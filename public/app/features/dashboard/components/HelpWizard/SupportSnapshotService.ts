...
export class SupportSnapshotService extends StateManagerBase<SupportSnapshotState> {
  ...
  async buildDebugDashboard() {
    ...
    try {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/app/features/dashboard/components/HelpWizard/SupportSnapshotService.ts', args: ['Error creating scene:', ex] }]);
    }
