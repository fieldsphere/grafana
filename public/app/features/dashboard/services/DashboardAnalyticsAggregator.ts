import { createStructuredLogger, logMeasurement, reportInteraction } from '@grafana/runtime';

const logger = createStructuredLogger('DashboardAnalyticsAggregator');
...
export class DashboardAnalyticsAggregator implements performanceUtils.ScenePerformanceObserver {
  ...
  onPanelOperationComplete = (data: performanceUtils.PanelPerformanceData): void => {
    ...
    if (!panel) {
      logger.warn('Panel not found for operation completion', { panelKey: data.panelKey });
      return;
    }
