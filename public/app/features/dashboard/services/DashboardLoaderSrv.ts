...
const logger = createMonitoringLogger('grafana.features.dashboard.DashboardLoaderSrv');
  ...
  protected loadScriptedDashboard(file: string) {
    ...
    return getBackendSrv()
      ...
      .then(
        ...
        (err) => {
          logger.logError('Script dashboard error', { error: err });
      ...
      promise = getDashboardAPI('v1')
        ...
        .catch((e) => {
          if (isFetchError(e) && !(e instanceof DashboardVersionError)) {
            logger.logError('Failed to load dashboard', { error: e });
            e.isHandled = true;
            if (e.status === 404) {
