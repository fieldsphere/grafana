...
import { getBackendSrv, isFetchError, locationService, logError } from '@grafana/runtime';
  ...
  protected loadScriptedDashboard(file: string) {
    ...
    return getBackendSrv()
      ...
      .then(
        ...
        (err) => {
          logError(err instanceof Error ? err : new Error(String(err)), { source: 'script-dashboard' });
      ...
      promise = getDashboardAPI('v1')
        ...
        .catch((e) => {
          if (isFetchError(e) && !(e instanceof DashboardVersionError)) {
            logError(e instanceof Error ? e : new Error(String(e)), { source: 'dashboard-loader-v1' });
            e.isHandled = true;
            if (e.status === 404) {
  ...
  loadDashboard(
    ...
    if (type === 'script' && slug) {
      ...
      promise = getDashboardAPI('v2')
        ...
        .catch((e) => {
          if (isFetchError(e) && !(e instanceof DashboardVersionError)) {
            logError(e instanceof Error ? e : new Error(String(e)), { source: 'dashboard-loader-v2' });
            e.isHandled = true;
            if (e.status === 404) {
              appEvents.emit(AppEvents.alertError, ['Dashboard not found']);
            }
