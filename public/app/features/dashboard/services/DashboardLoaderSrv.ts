...
const logger = createMonitoringLogger('features.dashboard.loader-service');
  ...
  protected loadScriptedDashboard(file: string) {
    ...
    return getBackendSrv()
      ...
      .then(
        ...
        (err) => {
          logger.logWarning('Script dashboard error', { error: String(err) });
      ...
      promise = getDashboardAPI('v1')
        ...
        .catch((e) => {
          if (isFetchError(e) && !(e instanceof DashboardVersionError)) {
            logger.logError(e, { operation: 'loadDashboard', version: 'v1', uid });
            e.isHandled = true;
            if (e.status === 404) {
  ...
  loadDashboard(
    ...
    if (type === 'script' && slug) {
      ...
      promise = getDashboardAPI('v2')
        ...
        .catch((e) => {
          if (isFetchError(e) && !(e instanceof DashboardVersionError)) {
            logger.logError(e, { operation: 'loadDashboard', version: 'v2', uid });
            e.isHandled = true;
            if (e.status === 404) {
              appEvents.emit(AppEvents.alertError, ['Dashboard not found']);
            }
