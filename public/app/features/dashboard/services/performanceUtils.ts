...
export function writePerformanceGroupLog(logger: string, message: string, data?: unknown): void {
  if (isPerformanceLoggingEnabled()) {
    if (data) {
      // eslint-disable-next-line no-console
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log(message, data);
    } else {
      // eslint-disable-next-line no-console
...
export function createPerformanceMark(name: string, timestamp?: number): void {
  try {
    ...
    (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error(`❌ Failed to create performance mark: ${name}`, { timestamp, error });
  }
}
...
export function createPerformanceMeasure(name: string, startMark: string, endMark?: string): void {
  try {
    ...
    (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error(`❌ Failed to create performance measure: ${name}`, { startMark, endMark, error });
  }
}
