    ...
    switch (v.type) {
      case 'query':
        ...
        if (typeof query === 'string') {
          logger.logWarning('Query variable query is a string which is deprecated in the schema v2. It should extend DataQuery');
          query = {
      ...
      default:
        ...
        logger.logError('Variable transformation not implemented', { variableType: v.type });
    }
  }
  return variables;
}
...
function getVariablesV1(vars: DashboardV2Spec['variables']): VariableModel[] {
  ...
  for (const v of vars) {
    ...
    switch (v.kind) {
      ...
      default:
        // do not throw error, just log it
        logger.logError('Variable transformation not implemented', { variable: v });
    }
  }
  return variables;
}
