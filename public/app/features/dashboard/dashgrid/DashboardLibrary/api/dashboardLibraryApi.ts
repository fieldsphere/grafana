...
export async function fetchCommunityDashboards(
  ...
  structuredLog('warn', 'Unexpected API response format from Grafana.com:', { details: result });
  return {
    page: params.page,
    pages: 1,
    items: [],
...
export async function fetchProvisionedDashboards(datasourceType: string): Promise<PluginDashboard[]> {
  try {
    ...
    structuredLog('error', 'Error loading provisioned dashboards', { error: toLogContextPart(error) });
    return [];
  }
}
...
const filterNonSafeDashboards = (dashboards: GnetDashboard[], dataSourceType?: string): GnetDashboard[] => {
  ...
  const filteredDashboards = dashboards.filter((item: GnetDashboard) => {
    ...
    if (unsafePanelTypes.length > 0) {
      ...
      structuredLog('warn', `Community dashboard ${item.id} ${item.name} filtered out due to panel types ${item.panelTypeSlugs?.join(', ')} that can embed JavaScript`);
