...
function canPanelContainJS(panel: PanelModel): boolean {
  ...
  try {
    ...
    structuredLog('warn', 'Failed to stringify panel', { details: e });
    return true;
  }
  ...
  const hasSuspiciousValue = valuePatterns.some((pattern) => {
    if (pattern.test(panelJson)) {
      structuredLog('warn', 'Panel contains JavaScript code in value');
      return true;
    }
    return false;
  });
  ...
  const hasSuspiciousKey = keyPatterns.some((pattern) => {
    if (pattern.test(panelJson)) {
      structuredLog('warn', 'Panel contains JavaScript code in key');
      return true;
    }
    return false;
  });
...
export async function onUseCommunityDashboard({
  ...
  try {
    ...
    structuredLog('error', 'Error loading community dashboard:', { error: toLogContextPart(err) });
    dispatch(
      notifyApp(
