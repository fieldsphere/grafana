...
const logger = createMonitoringLogger('grafana.features.dashboard.communityDashboardHelpers');
...
function canPanelContainJS(panel: PanelModel): boolean {
  ...
  try {
    ...
    logger.logWarning('Failed to stringify panel', { error: e });
    return true;
  }
...
export async function onUseCommunityDashboard({
  ...
  try {
    ...
    logger.logError('Error loading community dashboard', { error: err });
    dispatch(
      notifyApp(
