...
export class DashboardModel implements TimeModel {
  ...
  *panelIterator() {
  ...
  on<T>(event: AppEvent<T>, callback: (payload?: T) => void) {
    (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log('DashboardModel.on is deprecated use events.subscribe');
    this.events.on(event, callback);
  }

  /** @deprecated */
  off<T>(event: AppEvent<T>, callback: (payload?: T) => void) {
    (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log('DashboardModel.off is deprecated');
