...
const logger = createMonitoringLogger('grafana.features.dashboard.DashboardModel');
...
export class DashboardModel implements TimeModel {
  ...
  *panelIterator() {
  ...
  on<T>(event: AppEvent<T>, callback: (payload?: T) => void) {
    logger.logDebug('DashboardModel.on is deprecated use events.subscribe');
    this.events.on(event, callback);
  }

  /** @deprecated */
  off<T>(event: AppEvent<T>, callback: (payload?: T) => void) {
    logger.logDebug('DashboardModel.off is deprecated');
