...
export function RecentlyDeletedActions() {
  ...
  const onRestore = async (restoreTarget: string) => {
    ...
    const promises = selectedDashboards.map(async (uid) => {
      ...
      if (!dashboard) {
        Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'public/app/features/browse-dashboards/components/RecentlyDeletedActions.tsx', args: [`Dashboard ${uid} not found in deleted items`] }]);
