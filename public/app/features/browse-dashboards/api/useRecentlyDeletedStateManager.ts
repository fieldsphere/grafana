...
const logger = createMonitoringLogger('features.browse-dashboards.deleted-state-manager');
  ...
  getTagOptions = async (): Promise<TermCount[]> => {
    try {
      ...
      if (error instanceof Error) {
        logger.logError(error, { operation: 'getTagOptions' });
      } else {
        logger.logWarning('Failed to get tags from deleted dashboards', {
          operation: 'getTagOptions',
          error: String(error),
        });
      }
      return [];
