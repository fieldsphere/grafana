...
const logger = createMonitoringLogger('grafana.core.trustedTypePolicies');
...
export const defaultTrustedTypesPolicy = {
  createHTML: (string: string, source: string, sink: string) => {
    ...
    logger.logError(new Error('HTML not sanitized with Trusted Types'), { source, sink });
    return string;
  },
  ...
  createScriptURL: (string: string, source: string, sink: string) => {
    ...
    logger.logError(new Error('ScriptURL not sanitized with Trusted Types'), { source, sink });
    return string;
  },
};
