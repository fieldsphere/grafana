...
export function NavLandingPage({ navId, header }: Props) {
  ...
  React.useEffect(() => {
    if (components && components.length > 0 && additionalCards && additionalCards.length > 0) {
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'public/app/core/components/NavLandingPage/NavLandingPage.tsx', args: [
