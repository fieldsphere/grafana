...
const logger = createMonitoringLogger('grafana.core.explore');
...
export const safeStringifyValue = (value: unknown, space?: number) => {
  ...
  try {
    ...
    logger.logError(error instanceof Error ? error : new Error(String(error)), { operation: 'safeStringifyValue' });
  }
    ...
    for (let index = 0; index < queries.length; index++) {
      ...
      if (query.datasource) {
        try {
          ...
          logger.logError(new Error('One of the queries has a datasource that is no longer available and was removed'), { datasourceUid: query.datasource.uid });
          validDS = false;
        }
      }
