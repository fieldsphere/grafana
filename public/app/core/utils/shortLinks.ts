...
import { createStructuredLogger, getBackendSrv, config, locationService } from '@grafana/runtime';

const logger = createStructuredLogger('ShortLinks');
...
export const createShortLink = memoizeOne(async (path: string): Promise<string> => {
  try {
    ...
    logger.error('Error when creating shortened link', err instanceof Error ? err : undefined, { path });
...
export const createAndCopyShortLink = async (path: string) => {
  try {
    ...
    logger.error('Error in createAndCopyShortLink', error instanceof Error ? error : undefined, { path });
  }
};
