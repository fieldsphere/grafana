...
import { getBackendSrv, config, locationService, logError } from '@grafana/runtime';
...
export const createShortLink = memoizeOne(async (path: string): Promise<string> => {
  try {
    ...
    logError(err instanceof Error ? err : new Error(String(err)), {
      message: 'Error when creating shortened link',
    });
...
export const createAndCopyShortLink = async (path: string) => {
  try {
    ...
    logError(error instanceof Error ? error : new Error(String(error)), {
      message: 'Error in createAndCopyShortLink',
    });
  }
};
