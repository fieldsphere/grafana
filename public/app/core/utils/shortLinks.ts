...
import { getBackendSrv, config, locationService, createMonitoringLogger } from '@grafana/runtime';

const logger = createMonitoringLogger('grafana.core.shortLinks');
...
export const createShortLink = memoizeOne(async (path: string): Promise<string> => {
  try {
    ...
    logger.logError(err instanceof Error ? err : new Error(String(err)), { operation: 'createShortLink', path });
...
export const createAndCopyShortLink = async (path: string) => {
  try {
    ...
    logger.logError(error instanceof Error ? error : new Error(String(error)), { operation: 'createAndCopyShortLink', path });
  }
};
