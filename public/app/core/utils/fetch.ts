    ...
    switch (responseType) {
      ...
      case 'json':
        ...
        if (response.headers.get('Content-Length') === '0') {
          Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'public/app/core/utils/fetch.ts', args: [`${response.url} returned an invalid JSON`] }]);
          return {} as T;
        }
