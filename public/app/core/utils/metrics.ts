...
const logger = createMonitoringLogger('grafana.core.metrics');
...
export function startMeasure(eventName: string) {
  ...
  try {
    ...
    logger.logError(error instanceof Error ? error : new Error(String(error)), { eventName, operation: 'startMeasure' });
  }
}
...
export function stopMeasure(eventName: string) {
  ...
  try {
    ...
    logger.logError(error instanceof Error ? error : new Error(String(error)), { eventName, operation: 'stopMeasure' });
    return;
  }
}
