...
const logger = createMonitoringLogger('grafana.core.context_srv');
...
export class ContextSrv {
  ...
  async fetchUserPermissions() {
    try {
      ...
      logger.logError(e instanceof Error ? e : new Error('Error fetching user permissions'));
    }
  }
  ...
  private rotateToken() {
    ...
    return fetch(config.appSubUrl + '/api/user/auth-tokens/rotate', { method: 'POST' })
      ...
      .catch((e) => {
        logger.logError(e instanceof Error ? e : new Error('Error rotating token'));
      });
  }
}
