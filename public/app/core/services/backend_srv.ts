...
const logger = createMonitoringLogger('grafana.core.backend_srv');
...
export class BackendSrv implements BackendService {
  ...
  private async initGrafanaDeviceID() {
    try {
      ...
      logger.logError(error instanceof Error ? error : new Error('Error initializing Grafana device ID'));
    }
  }
    ...
    return {
      next: (response) => {
        ...
        process()
          ...
          .catch((e) => {
            logger.logError(e instanceof Error ? e : new Error(`Error processing chunked request ${requestId}`));
            observer.error(e);
          }); // from abort
      },
      error: (e) => {
