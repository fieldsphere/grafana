...
import { config, createStructuredLogger, getAppEvents } from '@grafana/runtime';
import { ScrollContainer, usePanelContext, useStyles2 } from '@grafana/ui';

const logger = createStructuredLogger('LogsPanel');
  ...
  const loadMoreLogs = useCallback(
    async (scrollRange: AbsoluteTimeRange) => {
      ...
      try {
        ...
        logger.error('Error loading more logs', e instanceof Error ? e : new Error(String(e)));
      } finally {
...
function getLogsPanelState(): LogsPermalinkUrlState | undefined {
  ...
  if (
    ...
    try {
      ...
      logger.error('Error parsing logsPanelState', e instanceof Error ? e : new Error(String(e)));
    }
  }
...
export async function requestMoreLogs(
  ...
  for (const uid in targetGroups) {
    ...
    if (!dataSource) {
      logger.warn('Could not resolve data source for target', { refId: panelData.request.targets[0].refId });
      continue;
    }
    dataRequests.push(
      dataSource.query({
