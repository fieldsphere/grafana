...
import { config, getAppEvents, createMonitoringLogger } from '@grafana/runtime';
import { ScrollContainer, usePanelContext, useStyles2 } from '@grafana/ui';

const logger = createMonitoringLogger('grafana.plugins.panel.logs');
...
function getLogsPanelState(): LogsPermalinkUrlState | undefined {
  ...
  if (
    ...
    try {
      ...
      logger.logError('error parsing logsPanelState', e);
    }
  }
...
export async function requestMoreLogs(
  ...
  for (const uid in targetGroups) {
    ...
    if (!dataSource) {
      logger.logWarning(`Could not resolve data source for target ${panelData.request.targets[0].refId}`);
      continue;
    }
    dataRequests.push(
      dataSource.query({
