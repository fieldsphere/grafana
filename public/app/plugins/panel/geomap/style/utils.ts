...
const logger = createMonitoringLogger('plugins.panel.geomap.style-utils');
...
export function getRGBValues(colorString: string): ColorValue | null {
  ...
  // Handle other color formats if needed
    ...
    logger.logWarning('Unsupported color format', { operation: 'getRGBValues', colorString });
  }
  return null;
}
...
function getRGBFromRGBString(rgbString: string): ColorValue | null {
  ...
  if (matches) {
    if (matches.length === 3) {
      ...
      logger.logWarning('Unsupported color format', { operation: 'getRGBFromRGBString', rgbString });
    }
  } else {
    logger.logWarning('Unsupported color format', { operation: 'getRGBFromRGBString', rgbString });
  }
