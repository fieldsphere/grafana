...
import {
  apply } from 'ol-mapbox-style';

import { type MapLayerRegistryItem, type MapLayerOptions, type GrafanaTheme2, type EventBus,
  structuredLog
} from '@grafana/data';
  ...
  create: async (
    ...
    init: () => {
      ...
      const loadStyle = async () => {
        try {
          if (!cfg.url) {
            structuredLog('warn', 'No URL provided for MapLibre style, layer will be empty');
            return;
          }

          const res = await fetch(cfg.url);
          if (!res.ok) {
            structuredLog('warn', `Failed to load MapLibre style from ${cfg.url}: ${res.status} ${res.statusText}`);
          ...
          structuredLog('warn', 'Failed to parse or apply MapLibre style JSON:', { details: error });
          // Try fallback approach
          await tryFallbackApply();
        }
      };
      ...
      const tryFallbackApply = async () => {
        try {
          if (!cfg.url) {
            structuredLog('warn', 'No URL available for MapLibre fallback, layer will be empty');
            return;
          }
          await apply(layer, cfg.url, { accessToken: cfg.accessToken });
          applyNoRepeat();
        } catch (fallbackError) {
          structuredLog('warn', 'Failed to load MapLibre style from both JSON and direct URL approaches:', { details: fallbackError });
        }
      };
