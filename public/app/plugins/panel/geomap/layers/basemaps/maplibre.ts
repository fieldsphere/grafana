...
const logger = createStructuredLogger('MaplibreLayer');
  ...
  create: async (
    ...
    init: () => {
      ...
      const loadStyle = async () => {
        try {
          if (!cfg.url) {
            logger.warn('No URL provided for MapLibre style, layer will be empty');
            return;
          }

          const res = await fetch(cfg.url);
          if (!res.ok) {
            logger.warn('Failed to load MapLibre style', { url: cfg.url, status: res.status, statusText: res.statusText });
      ...
      const tryFallbackApply = async () => {
        try {
          if (!cfg.url) {
            logger.warn('No URL available for MapLibre fallback, layer will be empty');
            return;
          }
          await apply(layer, cfg.url, { accessToken: cfg.accessToken });
          applyNoRepeat();
        } catch (fallbackError) {
          logger.warn('Failed to load MapLibre style from both JSON and direct URL approaches', { error: fallbackError });
        }
      };
