  ...
  create: async (
    ...
    init: () => {
      ...
      const loadStyle = async () => {
        try {
          if (!cfg.url) {
            structuredLogFromConsole('warn', 'No URL provided for MapLibre style, layer will be empty');
            return;
          }

          const res = await fetch(cfg.url);
          if (!res.ok) {
            structuredLogFromConsole('warn', `Failed to load MapLibre style from ${cfg.url}: ${res.status} ${res.statusText}`);
          ...
          structuredLogFromConsole('warn', 'Failed to parse or apply MapLibre style JSON:', error);
          // Try fallback approach
          await tryFallbackApply();
        }
      };
      ...
      const tryFallbackApply = async () => {
        try {
          if (!cfg.url) {
            structuredLogFromConsole('warn', 'No URL available for MapLibre fallback, layer will be empty');
            return;
          }
          await apply(layer, cfg.url, { accessToken: cfg.accessToken });
          applyNoRepeat();
        } catch (fallbackError) {
          structuredLogFromConsole('warn', 'Failed to load MapLibre style from both JSON and direct URL approaches:', fallbackError);
        }
      };
