  ...
  create: async (
    ...
    init: () => {
      ...
      const loadStyle = async () => {
        try {
          if (!cfg.url) {
            logger.logWarning('No URL provided for MapLibre style, layer will be empty');
            return;
          }

          const res = await fetch(cfg.url);
          if (!res.ok) {
            logger.logWarning(`Failed to load MapLibre style from ${cfg.url}: ${res.status} ${res.statusText}`);
          ...
          logger.logWarning('Failed to parse or apply MapLibre style JSON:', error);
          // Try fallback approach
          await tryFallbackApply();
        }
      };
      ...
      const tryFallbackApply = async () => {
        try {
          ...
          logger.logWarning('Failed to load MapLibre style from both JSON and direct URL approaches:', fallbackError);
        }
      };
