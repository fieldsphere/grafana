...
const logger = createMonitoringLogger('plugins.panel.table.migrations');
...
export const tableMigrationHandler = (panel: PanelModel<Options>): Partial<Options> => {
  ...
  if (!panel.pluginVersion && 'columns' in panel) {
    logger.logDebug('Angular table panel detected during migration', {
      operation: 'tableMigrationHandler',
      panelId: panel.id,
    });
  }
