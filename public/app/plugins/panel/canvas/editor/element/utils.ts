...
import { BackendSrvRequest, createStructuredLogger, getBackendSrv, getTemplateSrv } from '@grafana/runtime';
import { appEvents } from 'app/core/app_events';

const logger = createStructuredLogger('CanvasElementUtils');
  ...
  getBackendSrv()
    ...
    .subscribe({
      error: (error) => {
        ...
        logger.error('API call error', error instanceof Error ? error : new Error(String(error)), { endpoint: api.endpoint });
