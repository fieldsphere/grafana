...
import { config, getGrafanaLiveSrv, createMonitoringLogger } from '@grafana/runtime';
import { Alert, stylesFactory, JSONFormatter, CustomScrollbar } from '@grafana/ui';

const logger = createMonitoringLogger('grafana.plugins.panel.live');
...
export class LivePanel extends PureComponent<Props, State> {
  ...
  async loadChannel() {
    ...
    if (isEqual(addr, this.state.addr)) {
      logger.logDebug('Same channel', this.state.addr);
      return;
    }
