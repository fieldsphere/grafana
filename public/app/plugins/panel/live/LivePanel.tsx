...
import { config, getGrafanaLiveSrv, logStructuredDebug } from '@grafana/runtime';
...
export class LivePanel extends PureComponent<Props, State> {
  ...
  streamObserver: PartialObserver<LiveChannelEvent> = {
    next: (event: LiveChannelEvent) => {
      if (isLiveChannelStatusEvent(event)) {
        ...
        logStructuredDebug('plugins.panel.live', 'ignore', event);
      }
    },
  };
  ...
  async loadChannel() {
    ...
    if (!isValidLiveChannelAddress(addr)) {
      logStructuredDebug('plugins.panel.live', 'INVALID', addr);
      this.unsubscribe();
      this.setState({
        addr: undefined,
      });
    ...
    if (isEqual(addr, this.state.addr)) {
      logStructuredDebug('plugins.panel.live', 'Same channel', this.state.addr);
      return;
    }
    ...
    logStructuredDebug('plugins.panel.live', 'LOAD', addr);
