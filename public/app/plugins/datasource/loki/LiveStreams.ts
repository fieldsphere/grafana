import {
  type Observable, throwError, timer } from 'rxjs';
import { finalize, map, retryWhen, mergeMap } from 'rxjs/operators';
import { webSocket } from 'rxjs/webSocket';

import { type DataFrame, FieldType, type KeyValue, CircularDataFrame,
  structuredLog
} from '@grafana/data';
          ...
          mergeMap((error, i) => {
            ...
            if (error.code === 1006 && retryAttempt < 30) {
              if (retryAttempt > 10) {
                ...
                structuredLog('warn', `Websocket connection is being disrupted. We keep reconnecting but consider starting new live tailing again. Error: ${error.reason}`);
              }
              // Retry every 5s
