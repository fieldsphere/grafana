...
export function runSignalStream(
  ...
  return new Observable<DataQueryResponse>((subscriber) => {
    ...
    return () => {
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/app/plugins/datasource/grafana-testdata-datasource/runStreams.ts', args: ['unsubscribing to stream ' + streamId] }]);
      ...
      .subscribe({
        next: (chunk) => {
          if (!chunk.data || !chunk.ok) {
            Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/app/plugins/datasource/grafana-testdata-datasource/runStreams.ts', args: ['chunk missing data', chunk] }]);
            return;
          }
          decoder
            ...
            .forEach((line) => {
              if (line?.length) {
                try {
                  ...
                  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'public/app/plugins/datasource/grafana-testdata-datasource/runStreams.ts', args: ['error parsing line', line, err] }]);
                }
              }
            });
        },
        error: (err) => {
          Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'public/app/plugins/datasource/grafana-testdata-datasource/runStreams.ts', args: ['error in stream', streamId, err] }]);
        },
        complete: () => {
          Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/app/plugins/datasource/grafana-testdata-datasource/runStreams.ts', args: ['complete stream', streamId] }]);
        },
      });
...
export function runFetchStream(
  ...
  return new Observable<DataQueryResponse>((subscriber) => {
    ...
    const processChunk = async (
      ...
      if (value.done) {
        Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/app/plugins/datasource/grafana-testdata-datasource/runStreams.ts', args: ['Finished stream'] }]);
