...
const logger = createMonitoringLogger('plugins.datasource.grafana-testdata.raw-frame-editor');
...
export const RawFrameEditor = ({ onChange, query }: EditorProps) => {
  ...
  const onSaveFrames = (rawFrameContent: string) => {
    try {
      ...
      if (data) {
        logger.logDebug('Converted raw frame content to direct frame result', {
          operation: 'onSaveFrames',
      ...
      logger.logWarning('Error parsing JSON in raw frame editor', {
        operation: 'onSaveFrames',
        error: String(e),
      });
