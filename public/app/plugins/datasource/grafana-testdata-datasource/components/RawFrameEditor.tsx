...
import { dataFrameToJSON, toDataFrame, toDataFrameDTO,
  structuredLog
} from '@grafana/data';
...
export const RawFrameEditor = ({ onChange, query }: EditorProps) => {
  ...
  const onSaveFrames = (rawFrameContent: string) => {
    try {
      ...
      if (data) {
        structuredLog('info', 'Original', { details: json });
        structuredLog('info', 'Save', { details: data });
        setError(undefined);
      ...
      structuredLog('info', 'Error parsing json', { details: e });
