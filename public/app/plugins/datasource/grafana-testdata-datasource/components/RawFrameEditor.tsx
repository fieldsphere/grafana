...
import { createStructuredLogger, toDataQueryResponse } from '@grafana/runtime';
import { Alert, CodeEditor } from '@grafana/ui';

import { EditorProps } from '../QueryEditor';

const logger = createStructuredLogger('RawFrameEditor');
...
export const RawFrameEditor = ({ onChange, query }: EditorProps) => {
  ...
  const onSaveFrames = (rawFrameContent: string) => {
    try {
      ...
      if (data) {
        logger.debug('Frame editor data', { original: json, save: data });
        setError(undefined);
      ...
      logger.debug('Error parsing json', { error: e });
