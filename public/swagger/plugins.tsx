    ...
    wrapComponents: {
      parameterRow: (Original: React.ElementType) => (props: UntypedProps) => {
        ...
        if (name === 'name' && where === 'path') {
          ...
          if (path.length > 4 && path[1] === 'apis') {
            ...
            // structuredLog.info('NAME (in path)', path, info);
            return (
      ...
      RequestBody: (Original: React.ElementType) => (props: UntypedProps) => {
        ...
        if (content) {
          ...
          Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/swagger/plugins.tsx', args: ['RequestBody', v, mime, props] }]);
        }
        // structuredLog.info('RequestBody PROPS', props);
        return (
      ...
      modelExample: (Original: React.ElementType) => (props: UntypedProps) => {
        if (props.isExecute && props.schema) {
          Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/swagger/plugins.tsx', args: ['modelExample PROPS', props] }]);
          return (
          ...
          <SchemaContext.Consumer>
            {(schema) => {
              if (schema) {
                ...
                //structuredLog.info('JSON TextArea', props, info);
                ...
                Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'public/swagger/plugins.tsx', args: ['CodeEditor', schema] }]);
