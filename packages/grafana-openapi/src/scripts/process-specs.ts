...
function processDirectory(sourceDir: string, outputDir: string) {
  ...
  for (const file of files) {
    ...
    (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log(`Processing file "${file}"...`);
    ...
    try {
      ...
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error(`Invalid JSON file "${file}". Skipping this file.`);
      continue;
    }

    const outputSpec = processOpenAPISpec(inputSpec);
    fs.writeFileSync(outputPath, JSON.stringify(outputSpec, null, 2), 'utf-8');
    (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log(`Processing completed for file "${file}".`);
  }
}
