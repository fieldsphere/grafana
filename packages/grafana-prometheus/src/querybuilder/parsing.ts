...
export function buildVisualQueryFromString(expr: string): Omit<Context, 'replacements'> {
  ...
  try {
    ...
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'packages/grafana-prometheus/src/querybuilder/parsing.ts', args: [err] }]);
