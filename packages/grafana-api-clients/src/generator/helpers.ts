...
export const runGenerateApis =
  ...
  (answers, config) => {
    try {
      ...
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log(`⏳ Running ${command} to generate endpoints...`);
      execSync(command, { stdio: 'inherit', cwd: basePath });
      return '✅ API endpoints generated successfully!';
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error('❌ Failed to generate API endpoints:', errorMessage);
...
export const formatFiles =
  ...
  (_, config) => {
    if (!config || !Array.isArray(config.files)) {
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error('Invalid config passed to formatFiles action');
    ...
    try {
      ...
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log('🧹 Running ESLint on generated/modified files...');
      try {
        execSync(`yarn eslint --fix ${filesList}`, { cwd: basePath });
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).warn(`⚠️ Warning: ESLint encountered issues: ${errorMessage}`);
      }

      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log('🧹 Running Prettier on generated/modified files...');
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).warn(`⚠️ Warning: Prettier encountered issues: ${errorMessage}`);
      }

      return '✅ Files linted and formatted successfully!';
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error('⚠️ Warning: Formatting operations failed:', errorMessage);
...
export const updatePackageJsonExports =
  ...
  (answers) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).error('❌ Failed to update package.json exports:', errorMessage);
