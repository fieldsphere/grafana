...
export const runGenerateApis =
  ...
  (answers, config) => {
    try {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: [`⏳ Running ${command} to generate endpoints...`] }]);
      execSync(command, { stdio: 'inherit', cwd: basePath });
      return '✅ API endpoints generated successfully!';
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: ['❌ Failed to generate API endpoints:', errorMessage] }]);
...
export const formatFiles =
  ...
  (_, config) => {
    if (!config || !Array.isArray(config.files)) {
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: ['Invalid config passed to formatFiles action'] }]);
    ...
    try {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: ['🧹 Running ESLint on generated/modified files...'] }]);
      try {
        execSync(`yarn eslint --fix ${filesList}`, { cwd: basePath });
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: [`⚠️ Warning: ESLint encountered issues: ${errorMessage}`] }]);
      }

      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: ['🧹 Running Prettier on generated/modified files...'] }]);
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: [`⚠️ Warning: Prettier encountered issues: ${errorMessage}`] }]);
      }

      return '✅ Files linted and formatted successfully!';
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: ['⚠️ Warning: Formatting operations failed:', errorMessage] }]);
...
export const updatePackageJsonExports =
  ...
  (answers) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'packages/grafana-api-clients/src/generator/helpers.ts', args: ['❌ Failed to update package.json exports:', errorMessage] }]);
