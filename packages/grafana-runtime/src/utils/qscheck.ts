import { type DataSourceInstanceSettings, type DataSourceJsonData, structuredLog } from '@grafana/data';
...
function parseAllowedTypes(data: unknown): AllowedTypes {
  ...
  if (data != null && typeof data === 'object' && 'types' in data && Array.isArray(data.types)) {
    ...
    if (types.every((x) => typeof x === 'string')) {
      ...
      structuredLog('error', 'qscheck.parseFlags: non-string item in allowed');
      return { types: [] };
    }
  } else {
    structuredLog('error', 'qscheck.parseFlags: invalid data');
    return { types: [] };
  }
}
