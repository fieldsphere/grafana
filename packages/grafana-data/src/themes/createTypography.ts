...
const typographyLogger = createStructuredLogger('grafana-data.create-typography');
...
export function createTypography(colors: ThemeColors, typographyInput: ThemeTypographyInput = {}): ThemeTypography {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (typeof fontSize !== 'number') {
      typographyLogger.logError('`fontSize` is required to be a number.');
    }

    if (typeof htmlFontSize !== 'number') {
      typographyLogger.logError('`htmlFontSize` is required to be a number.');
    }
  }
