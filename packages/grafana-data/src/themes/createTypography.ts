...
export function createTypography(colors: ThemeColors, typographyInput: ThemeTypographyInput = {}): ThemeTypography {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (typeof fontSize !== 'number') {
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'packages/grafana-data/src/themes/createTypography.ts', args: ['Grafana-UI: `fontSize` is required to be a number.'] }]);
    }

    if (typeof htmlFontSize !== 'number') {
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'error'), console, [{ timestamp: new Date().toISOString(), level: 'error', source: 'packages/grafana-data/src/themes/createTypography.ts', args: ['Grafana-UI: `htmlFontSize` is required to be a number.'] }]);
    }
  }
