...
const createSpacingLogger = createStructuredLogger('grafana-data.create-spacing');
...
export function createSpacing(options: ThemeSpacingOptions = {}): ThemeSpacing {
  ...
  const transform = (value: ThemeSpacingArgument) => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (typeof value !== 'number') {
        createSpacingLogger.logError('Expected spacing argument to be a number or a string', { value });
      }
    }
  ...
  const spacing = (...args: Array<number | string>): string => {
    if (process.env.NODE_ENV !== 'production') {
      if (!(args.length <= 4)) {
        createSpacingLogger.logError('Too many arguments provided, expected between 0 and 4', {
