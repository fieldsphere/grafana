...
export class EventBusSrv implements EventBus, LegacyEmitter {
  ...
  emit<T>(event: AppEvent<T> | string, payload?: T): void {
    // (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log(`Deprecated emitter function used (emit), use $emit`);
  ...
  on<T>(event: AppEvent<T> | string, handler: LegacyEventHandler<T>) {
    // (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log(`Deprecated emitter function used (on), use $on`);
