...
export function EmotionPerfTest() {
  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'packages/grafana-ui/src/components/ThemeDemos/EmotionPerfTest.tsx', args: ['process.env.NODE_ENV', process.env.NODE_ENV] }]);
...
function MeasureRender({ children, id }: { children: React.ReactNode; id: string }) {
  const onRender: ProfilerOnRenderCallback = (id, phase, actualDuration, baseDuration, startTime, commitTime) => {
    Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'packages/grafana-ui/src/components/ThemeDemos/EmotionPerfTest.tsx', args: ['Profile ' + id, actualDuration] }]);
  };
