import {
  isIconName, type SelectableValue,
  structuredLog
} from '@grafana/data';
...
export const selectableValueToComboboxOption = <T extends string | number>(
  ...
  if (v == null || v.value == null) {
    structuredLog('warn', 'selectableValueToComboboxOption: value is null or undefined', { details: v });
    return undefined;
  }
  if (v.icon != null && !isIconName(v.icon)) {
    structuredLog('warn', 'selectableValueToComboboxOption: icon is not a valid icon name', { details: v.icon });
    return undefined;
  }
  return {
    label: v.label,
