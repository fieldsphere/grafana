    ...
    setupNodeEvents(on, config) {
      ...
      on('task', {
        log({ message, optional }) {
          optional ? (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log(message, optional) : (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log(message);
          return null;
        },
      });
      ...
      on('before:browser:launch', (browser = {}, launchOptions) => {
        (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log('launching browser %s is headless? %s', browser.name, browser.isHeadless);
        ...
        (Reflect.get(globalThis, '__grafanaStructuredConsole') ?? console).log('setting the browser window size to %d x %d', width, height);
