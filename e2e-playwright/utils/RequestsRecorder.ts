...
export class RequestsRecorder {
  ...
  listen() {
    ...
    return () => {
      ...
      Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'e2e-playwright/utils/RequestsRecorder.ts', args: ['waiting for', this.#requestsInFlight, 'requests to finish'] }]);
  ...
  async #handleResponse(response: Response) {
    ...
    if (type === 'document') {
      if (this.#documentUrl) {
        Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'warn'), console, [{ timestamp: new Date().toISOString(), level: 'warn', source: 'e2e-playwright/utils/RequestsRecorder.ts', args: ['recieved additional document response', url] }]);
      }
