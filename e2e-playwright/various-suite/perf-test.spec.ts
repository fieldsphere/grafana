...
test('payload-size', { tag: '@performance' }, async ({ page }) => {
  ...
  Reflect.apply(Reflect.get(globalThis, '__structuredLog') ?? Reflect.get(console, 'info'), console, [{ timestamp: new Date().toISOString(), level: 'info', source: 'e2e-playwright/various-suite/perf-test.spec.ts', args: [metricsText] }]);
