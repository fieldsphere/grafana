...
You can also run `./scripts/verify-structured-logging-closeout.sh --help` to view all options with the current available mode list.
...
You can do the same through `make`:

```sh
make verify-structured-logging-closeout-mode mode=tests-only-quick
```

To preview commands without executing tests or probes, use:
...
Matrix mode executes the same named modes shown by `--list-modes`.
