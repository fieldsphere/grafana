...
func run(ctx context.Context, cmd *cli.Command) error {
	...
	slog.Debug("configuration", "grafanaDir", grafanaDir, "targz", targzPath, "licensePath", licensePath)
	...
	slog.Info("e2e test suite completed", "exitCode", code)
	...
	if code != 0 {
		if stdout, _ := c.Stdout(ctx); len(stdout) > 0 {
			slog.Error("e2e test suite failed", "stdout", stdout)
		}
