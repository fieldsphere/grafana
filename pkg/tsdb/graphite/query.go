...
func (s *Service) parseResponse(res *http.Response) ([]TargetResponseDTO, error) {
	...
	if res.StatusCode/100 != 2 {
		...
		s.logger.Info("Request failed", "status", res.Status, "error", graphiteError, "responseBody", string(body))
	...
	if err != nil {
		...
		if err != nil {
			s.logger.Info("Failed to unmarshal legacy graphite response", "error", err, "status", res.Status, "responseBody", string(body))
			return nil, backend.PluginError(err)
		}
