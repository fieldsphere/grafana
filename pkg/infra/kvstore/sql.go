...
func (kv *kvStoreSQL) Get(ctx context.Context, orgId int64, namespace string, key string) (string, bool, error) {
	...
	err := kv.sqlStore.WithDbSession(ctx, func(dbSession *db.Session) error {
		...
		if err != nil {
			kv.log.Debug("error getting kvstore value", "orgID", orgId, "namespace", namespace, "kvKey", key, "error", err)
			return err
		}
		if !has {
			kv.log.Debug("kvstore value not found", "orgID", orgId, "namespace", namespace, "kvKey", key)
			return nil
		}
		itemFound = true
