	...
	if err != nil {
		...
		for _, s := range v.createdSecureValues {
			if e := secrets.DeleteWhenOwnedByResource(ctx, v.ref, s); e != nil {
				logging.FromContext(ctx).Warn("unable to clean up new secure value", "name", s, "error", e)
			}
		}
		return err
	}
...
func (s *Storage) prepareObjectForUpdate(ctx context.Context, updateObject runtime.Object, previousObject runtime.Object) (objectForStorage, error) {
	...
	if previous.GetUID() == "" {
		klog.ErrorS(errors.New("object is missing UID"), "Object is missing UID", "groupVersionKind", obj.GetGroupVersionKind().String(), "name", obj.GetName())
