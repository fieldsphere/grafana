...
decode:
	for {
		...
		case err != nil:
			klog.Error("client: error receiving result", "error", err)
			return watch.Error, nil, err
		}
		...
		if evt.Resource == nil {
			klog.Error("client: received nil")
			continue decode
		}
		...
		if evt.Type == resourcepb.WatchEvent_BOOKMARK {
			...
			accessor, err := utils.MetaAccessor(obj)
			if err != nil {
				klog.Error("error getting object accessor", "error", err)
				return watch.Error, nil, err
			}
		...
		if err != nil {
			klog.Error("error decoding entity", "error", err)
			return watch.Error, nil, err
		}
		...
		case resourcepb.WatchEvent_ADDED:
			...
			if err != nil {
				klog.Error("error matching object", "error", err)
				return watch.Error, nil, err
			}
			if !matches {
				continue decode
...
func (d *streamDecoder) Close() {
	...
	if err != nil {
		klog.Error("error closing watch stream", "error", err)
	}
