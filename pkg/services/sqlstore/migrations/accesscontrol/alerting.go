...
func (m *alertingMigrator) migrateNotificationActions() error {
	...
	if len(toAdd) > 0 {
		...
		m.migrator.Logger.Debug("updated roles with new permission", "added", added, "total", len(toAdd), "permission", accesscontrol.ActionAlertingNotificationsWrite)
	}
...
func (m *scopedReceiverTestingPermissions) Exec(sess *xorm.Session, mg *migrator.Migrator) error {
	...
	if err == nil {
		for id, ids := range rolesAffected {
			mg.Logger.Debug("Added permission to managed role", "permission", accesscontrol.ActionAlertingReceiversTestCreate, "roleID", id, "identifiers", ids)
		}
	}
	return err
}
