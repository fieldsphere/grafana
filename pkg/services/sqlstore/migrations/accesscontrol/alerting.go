...
func (m *alertingMigrator) migrateNotificationActions() error {
	...
	for roleID, add := range groupByRoleID {
		if !add {
			m.migrator.Logger.Info("Skip adding action to role because it already exists", "action", accesscontrol.ActionAlertingNotificationsWrite, "roleId", roleID)
			continue
		}
	...
	if len(toAdd) > 0 {
		...
		m.migrator.Logger.Debug("Updated roles with new permission", "updated", added, "total", len(toAdd), "permission", accesscontrol.ActionAlertingNotificationsWrite)
	}
