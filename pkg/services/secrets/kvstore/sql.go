...
func (kv *SecretsKVStoreSQL) Get(ctx context.Context, orgId int64, namespace string, typ string) (string, bool, error) {
	...
	err := kv.sqlStore.WithDbSession(ctx, func(dbSession *db.Session) error {
		...
		if err != nil {
			kv.log.Error("error getting secret value", "orgID", orgId, "secretType", typ, "namespace", namespace, "error", err)
			return err
		}
		if !has {
			kv.log.Debug("secret value not found", "orgID", orgId, "secretType", typ, "namespace", namespace)
			return nil
		}
		isFound = true
		return nil
	...
	if err == nil && isFound {
		...
		if err != nil {
			kv.log.Error("error decrypting secret value", "orgID", item.OrgId, "secretType", item.Type, "namespace", item.Namespace, "error", err)
			return string(decryptedValue), isFound, err
		}
	}

	kv.log.Debug("got secret value", "orgID", orgId, "secretType", typ, "namespace", namespace)
	return string(decryptedValue), isFound, err
}
...
func (kv *SecretsKVStoreSQL) Set(ctx context.Context, orgId int64, namespace string, typ string, value string) error {
	...
	if err != nil {
		kv.log.Error("error encrypting secret value", "orgID", orgId, "secretType", typ, "namespace", namespace, "error", err)
		return err
	}
	...
	return kv.sqlStore.WithTransactionalDbSession(ctx, func(dbSession *db.Session) error {
		...
		has, err := dbSession.Get(&item)
		if err != nil {
			kv.log.Error("error checking secret value", "orgID", orgId, "secretType", typ, "namespace", namespace, "error", err)
			return err
		}

		if has && item.Value == encodedValue {
			kv.log.Debug("secret value not changed", "orgID", orgId, "secretType", typ, "namespace", namespace)
			return nil
		}
		...
		if has {
			...
			if err != nil {
				kv.log.Error("error updating secret value", "orgID", orgId, "secretType", typ, "namespace", namespace, "error", err)
			} else {
				...
				kv.log.Debug("secret value updated", "orgID", orgId, "secretType", typ, "namespace", namespace)
			}
			return err
		}
		...
		if err != nil {
			kv.log.Error("error inserting secret value", "orgID", orgId, "secretType", typ, "namespace", namespace, "error", err)
		} else {
			kv.log.Debug("secret value inserted", "orgID", orgId, "secretType", typ, "namespace", namespace)
		}
		return err
	})
}
...
func (kv *SecretsKVStoreSQL) Del(ctx context.Context, orgId int64, namespace string, typ string) error {
	err := kv.sqlStore.WithDbSession(ctx, func(dbSession *db.Session) error {
		...
		if has {
			...
			if err != nil {
				kv.log.Error("error deleting secret value", "orgID", orgId, "secretType", typ, "namespace", namespace, "error", err)
			} else {
				kv.decryptionCache.Lock()
				defer kv.decryptionCache.Unlock()
				delete(kv.decryptionCache.cache, item.Id)
				kv.log.Debug("secret value deleted", "orgID", orgId, "secretType", typ, "namespace", namespace)
			}
			return err
		}
		return nil
...
func (kv *SecretsKVStoreSQL) Rename(ctx context.Context, orgId int64, namespace string, typ string, newNamespace string) error {
	return kv.sqlStore.WithTransactionalDbSession(ctx, func(dbSession *db.Session) error {
		...
		if has {
			...
			_, err = dbSession.ID(item.Id).Update(&item)
			if err != nil {
				kv.log.Error("error updating secret namespace", "orgID", orgId, "secretType", typ, "namespace", namespace, "error", err)
			} else {
				kv.log.Debug("secret namespace updated", "orgID", orgId, "secretType", typ, "namespace", namespace)
			}
			return err
		}
...
func (kv *SecretsKVStoreSQL) GetAll(ctx context.Context) ([]Item, error) {
	...
	if err != nil {
		kv.log.Error("error getting all the items", "error", err)
		return nil, err
	}
	...
	for i := range items {
		...
		if err != nil {
			kv.log.Error("error decrypting secret value", "orgID", items[i].OrgId, "secretType", items[i].Type, "namespace", items[i].Namespace, "error", err)
		}
	}
