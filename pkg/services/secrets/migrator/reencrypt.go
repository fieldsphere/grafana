...
func (s simpleSecret) ReEncrypt(ctx context.Context, secretsSrv *manager.SecretsService, sqlStore db.DB) bool {
	...
	if anyFailure {
		logger.Warn("Column re-encrypted with errors", "column", s.columnName, "table", s.tableName)
	} else {
		logger.Info("Column re-encrypted successfully", "column", s.columnName, "table", s.tableName)
	}
...
func (s jsonSecret) ReEncrypt(ctx context.Context, secretsSrv *manager.SecretsService, sqlStore db.DB) bool {
	...
	if anyFailure {
		logger.Warn("Secure json data secrets re-encrypted with errors", "table", s.tableName)
	} else {
		logger.Info("Secure json data secrets re-encrypted successfully", "table", s.tableName)
	}
