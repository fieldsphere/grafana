	...
	for {
		...
		case msg := <-ns.mailQueue:
			...
			if err != nil {
				...
				ns.log.Error("Async sent email partially failed", "sent_count", num, "recipients", tos, "info", info, "error", err)
			} else {
				ns.log.Debug("Async sent email succeeded", "sent_count", num, "recipients", tos, "info", info)
			}
		case <-ctx.Done():
			return ctx.Err()
		}
