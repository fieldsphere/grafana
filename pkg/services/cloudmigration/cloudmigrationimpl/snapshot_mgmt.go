...
func (s *Service) buildSnapshot(
	...
	start := time.Now()
	defer func() {
		s.log.Debug("buildSnapshot: method completed", "duration_ms", time.Since(start).Milliseconds())
	}()
	...
	s.log.Debug("buildSnapshot: generated keys", "duration_ms", time.Since(start).Milliseconds())
	...
	s.log.Debug("buildSnapshot: created snapshot writer", "duration_ms", time.Since(start).Milliseconds())
	...
	s.log.Debug("buildSnapshot: got migration data json", "duration_ms", time.Since(start).Milliseconds())
	...
	case cloudmigration.ResourceStorageTypeDb:
		...
		s.log.Debug("buildSnapshot: wrote data partitions with database storage", "duration_ms", time.Since(start).Milliseconds())
	...
	case cloudmigration.ResourceStorageTypeFs:
		...
		s.log.Debug("buildSnapshot: wrote data partitions with file system storage", "duration_ms", time.Since(start).Milliseconds())
...
func (s *Service) uploadSnapshot(ctx context.Context, session *cloudmigration.CloudMigrationSession, snapshotMeta *cloudmigration.CloudMigrationSnapshot, uploadUrl string) (err error) {
	...
	start := time.Now()
	defer func() {
		s.log.Debug("uploadSnapshot: method completed", "duration_ms", time.Since(start).Milliseconds())
	}()
...
func (s *Service) uploadSnapshotWithFSStorage(ctx context.Context, session *cloudmigration.CloudMigrationSession, snapshotMeta *cloudmigration.CloudMigrationSnapshot, uploadUrl string) error {
	...
	for _, fileNames := range index.Items {
		for _, fileName := range fileNames {
			...
			s.log.Debug("uploadSnapshot: uploaded file", "filename", fileName, "duration_ms", time.Since(start).Milliseconds())
		}
	}
	uploadSpan.End()

	s.log.Debug("uploadSnapshot: uploaded all data files", "duration_ms", time.Since(start).Milliseconds())
	...
	s.log.Debug("uploadSnapshot: uploaded index file", "duration_ms", time.Since(start).Milliseconds())
