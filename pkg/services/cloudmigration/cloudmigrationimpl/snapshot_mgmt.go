...
func (s *Service) updateSnapshotWithRetries(ctx context.Context, cmd cloudmigration.UpdateSnapshotCmd) (err error) {
	...
	if err := retryer.Retry(func() (retryer.RetrySignal, error) {
		...
		s.log.Error("failed to update snapshot status", "snapshotUID", cmd.UID, "status", cmd.Status, "numLocalResources", len(cmd.LocalResourcesToCreate), "numCloudResources", len(cmd.CloudResourcesToUpdate), "error", err)
