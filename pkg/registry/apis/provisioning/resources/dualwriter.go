...
func (r *DualReadWriter) createOrUpdate(ctx context.Context, create bool, opts DualWriteOptions) (*ParsedResource, error) {
	...
	if !opts.SkipDryRun {
		if err := parsed.DryRun(ctx); err != nil {
			logger := logging.FromContext(ctx).With("resourcePath", opts.Path, "resourceName", parsed.Obj.GetName(), "gitRef", opts.Ref)
...
func (r *DualReadWriter) moveFile(ctx context.Context, opts DualWriteOptions) (*ParsedResource, error) {
	...
	if !opts.SkipDryRun {
		if err := newParsed.DryRun(ctx); err != nil {
			logger := logging.FromContext(ctx).With("resourcePath", opts.Path, "originalPath", opts.OriginalPath, "resourceName", newParsed.Obj.GetName(), "gitRef", opts.Ref)
